...
/* Quicksand — the exact font used on Frappé theme (identified via DevTools) */
const cormorantFont = { fontFamily: "var(--font-quicksand), sans-serif" };
/* ─────────────────────────────────────────────
   SVG fruit outline icons (Frappé style)
───────────────────────────────────────────── */
const AppleIcon = () => (
  <svg viewBox="0 0 80 90" fill="none" stroke="white" strokeWidth="2" className="w-14 h-14 mx-auto mb-4 opacity-80">
    <path d="M40 12 C35 6 28 8 28 8 C28 8 30 2 40 2 C50 2 52 8 52 8 C52 8 45 6 40 12Z" />
    <path d="M20 30 C10 30 8 50 12 62 C16 74 22 82 30 82 C35 82 38 78 40 78 C42 78 45 82 50 82 C58 82 64 74 68 62 C72 50 70 30 60 30 C55 30 52 34 40 34 C28 34 25 30 20 30Z" />
  </svg>
);

const PearIcon = () => (
  <svg viewBox="0 0 80 95" fill="none" stroke="white" strokeWidth="2" className="w-14 h-14 mx-auto mb-4 opacity-80">
    <path d="M40 10 C38 4 40 2 40 2 C40 2 42 4 40 10Z" />
    <path d="M40 10 C40 10 36 14 35 20 C30 18 22 20 18 28 C12 38 14 55 22 66 C28 74 34 80 40 80 C46 80 52 74 58 66 C66 55 68 38 62 28 C58 20 50 18 45 20 C44 14 40 10 40 10Z" />
  </svg>
);

const OrangeIcon = () => (
  <svg viewBox="0 0 80 90" fill="none" stroke="white" strokeWidth="2" className="w-14 h-14 mx-auto mb-4 opacity-80">
    <path d="M40 8 C38 2 40 0 40 0 C40 0 42 2 40 8Z" />
    <circle cx="40" cy="48" r="30" />
    <path d="M40 18 L40 78" /><path d="M10 48 L70 48" />
    <path d="M18 28 L62 68" /><path d="M62 28 L18 68" />
  </svg>
);

const CherryIcon = () => (
  <svg viewBox="0 0 80 90" fill="none" stroke="white" strokeWidth="2" className="w-14 h-14 mx-auto mb-4 opacity-80">
    <path d="M40 10 C40 10 50 5 60 8 C70 11 68 20 60 24 C52 28 42 25 40 32" />
    <path d="M40 10 C40 10 30 5 20 8 C10 11 12 20 20 24 C28 28 38 25 40 32" />
    <circle cx="28" cy="62" r="14" />
    <circle cx="52" cy="66" r="14" />
  </svg>
);

/* ─────────────────────────────────────────────
   Wavy / torn-edge SVG dividers
───────────────────────────────────────────── */
/* ─────────────────────────────────────────────
   Frappé Authentic Organic Torn Paper Edge SVG Divider
───────────────────────────────────────────── */
const TornPaperBottom = ({ color = "#ffffff" }) => (
  <div className="absolute bottom-0 left-0 right-0 w-full overflow-hidden leading-none pointer-events-none z-20">
    <svg
      className="w-full h-[24px] sm:h-[32px] md:h-[42px] lg:h-[50px] block"
    ...
    >
      ...
      c-1.008-0.608-3.135-0.79-4.648,0.541c-0.876,0.771-2.188,0.289-3.313,0.232c-0.589-0.589-1.179-1.181-1.768-1.771
...
const WavyBottom = ({ color = "white" }) => (
  <div className="absolute bottom-0 left-0 right-0 w-full overflow-hidden leading-none pointer-events-none z-20">
    <svg viewBox="0 0 1440 80" preserveAspectRatio="none" className="w-full h-12 md:h-20" style={{ fill: color }}>
      <path d="M0,40 C180,80 360,0 540,40 C720,80 900,10 1080,45 C1260,80 1380,20 1440,40 L1440,80 L0,80 Z" />
    </svg>
  </div>
);

const WavyTop = ({ color = "white" }) => (
  <div className="absolute top-0 left-0 right-0 w-full overflow-hidden leading-none pointer-events-none z-20">
    <svg viewBox="0 0 1440 80" preserveAspectRatio="none" className="w-full h-12 md:h-20" style={{ fill: color }}>
      <path d="M0,40 C180,0 360,80 540,40 C720,0 900,70 1080,35 C1260,0 1380,60 1440,40 L1440,0 L0,0 Z" />
    </svg>
  </div>
);
...
const testimonials = [
  {
    quote: "Zussioberry blends are the freshest I've ever tasted — pure fruit goodness in every sip!",
    name: "Priya Sharma",
    role: "Smoothie Lover",
  },
  {
    quote: "Love the natural ingredients and zero preservatives. My family visits every week!",
    name: "Rahul Mehta",
    role: "Health Enthusiast",
  },
  {
    quote: "The FOCO model made franchising easy and profitable. Best business decision I made.",
    name: "Anita Desai",
...
/* ─────────────────────────────────────────────
   Animated Counter Component (Frappé style)
───────────────────────────────────────────── */
const AnimatedCounter = ({ target, duration = 2000 }) => {
  ...
  return (
    <span
      ref={ref}
      className="eltdf-counter eltdf-zero-counter font-bold block mb-1 text-4xl md:text-5xl"
...
export default function AboutPage() {
  ...
  return (
    <div className="w-full bg-white overflow-x-hidden font-sans text-gray-800">

      {/* ══════════════════════════════════════
          1. HERO SECTION  (bg-[#abe9b0] + pulse decorative elements)
      ══════════════════════════════════════ */}
      <section
        className="relative min-h-[65vh] md:min-h-[75vh] flex flex-col items-center justify-center text-center px-4 pt-28 pb-32 overflow-hidden bg-[#abe9b0]"
      >
        {/* Floating decorative elements with pulse animation */}
        <div className="absolute top-12 left-10 w-32 h-32 bg-white/25 rounded-full blur-2xl animate-pulse pointer-events-none" />
        <div className="absolute bottom-20 right-12 w-44 h-44 bg-white/35 rounded-full blur-3xl animate-pulse pointer-events-none" style={{ animationDuration: "3s" }} />
        <div className="absolute top-1/4 right-12 md:right-24 opacity-30 animate-pulse pointer-events-none text-4xl md:text-5xl select-none" style={{ animationDuration: "2.5s" }}>
          🍃
        </div>
        <div className="absolute bottom-1/3 left-10 md:left-20 opacity-30 animate-pulse pointer-events-none text-4xl md:text-5xl select-none" style={{ animationDuration: "3.5s" }}>
          🌱
        </div>

        <div className="relative z-10 max-w-2xl mx-auto text-left md:text-center md:max-w-lg">
          <h1
            className="text-5xl md:text-7xl lg:text-8xl font-light text-gray-800 mb-6 drop-shadow-sm"
            style={{ fontFamily: "var(--font-quicksand), sans-serif", fontWeight: 300, letterSpacing: "0.02em" }}
          >
            About Us
          </h1>
          <p className="text-gray-700 text-base md:text-lg font-light leading-relaxed">
            From a tiny juice kiosk to a growing national brand — our story is brewed with passion, purity and flavor.
          </p>
        </div>

        {/* torn paper edge SVG bottom divider */}
        <TornPaperBottom color="white" />
      </section>

      {/* ══════════════════════════════════════
          2. FEATURES GRID  (white bg)
      ══════════════════════════════════════ */}
      <section className="py-20 px-4 max-w-6xl mx-auto">
        <div className="text-center mb-14">
          <h2 className="text-3xl md:text-5xl font-light text-gray-800 mb-3" style={{ fontFamily: "var(--font-quicksand), sans-serif" }}>
            What Makes Us <span className="text-[#23aa5d]">Unique</span>?
          </h2>
          <p className="text-gray-400 max-w-md mx-auto text-sm italic" style={{ fontFamily: "cursive" }}>
            Blending innovation with natural purity for superior experiences
          </p>
        </div>

        <div className="vc_row wpb_row vc_inner vc_row-fluid vc_custom_1524064190538">
          <div className="wpb_column vc_column_container vc_col-sm-12">
            <div className="vc_column-inner">
              <div className="wpb_wrapper">
                <div
                  className="eltdf-item-showcase-holder clearfix grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12 items-center"
                  style={{ opacity: 1 }}
                >
                  {/* Left items holder */}
                  ...
                  >
                    <div className="relative w-56 md:w-72 lg:w-80 transition-transform duration-500 hover:scale-105">
                      <img
                        width={600}
                        height={764}
                        src="https://frappe.qodeinteractive.com/wp-content/uploads/2018/04/h3-img-3.png"
      ...
      >
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-10 relative z-10">
          {[
            { Icon: AppleIcon, num: "6412", label: "Cups Served Daily" },
            { Icon: PearIcon,  num: "5714", label: "Happy Customers" },
            { Icon: OrangeIcon, num: "3654", label: "Blends Created" },
            { Icon: CherryIcon, num: "7413", label: "Outlets Supported" },
          ].map(({ Icon, num, label }, i) => (
            <div key={i} className="flex flex-col items-center justify-center">
              <Icon />
              <AnimatedCounter target={num} />
              <div className="text-xs uppercase tracking-widest text-white/70 font-medium">{label}</div>
            </div>
          ))}
        </div>
      ...
      {/* ══════════════════════════════════════
          5. TESTIMONIALS  (white → wavy → mint)
      ══════════════════════════════════════ */}
      <section className="py-20 px-4 text-center">
        <h2 className="text-3xl md:text-5xl font-light text-gray-800 mb-3" style={{ fontFamily: "var(--font-quicksand), sans-serif" }}>They Say</h2>
        <p className="text-gray-400 text-sm italic mb-12" style={{ fontFamily: "cursive" }}>
          Voices of the Zussioberry community
        </p>
            ...
            {testimonials.map((t, i) => (
              <div
                ...
                className={`transition-all duration-700 absolute inset-0 flex flex-col items-center justify-center ${
                  i === activeSlide ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"
                }`}
              >
                <p
                  className="text-gray-500 text-base md:text-lg leading-loose mb-6 italic"
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                  i === activeSlide ? "bg-[#23aa5d] scale-125" : "bg-gray-300"
                }`}
              />
            ))}
          </div>
      ...
      {/* ══════════════════════════════════════
          6. BRAND / VALUES STRIP  (mint green)
      ══════════════════════════════════════ */}
      <section
        className="relative py-24 px-4 text-white overflow-hidden"
        style={{ background: "linear-gradient(135deg,#1a8a48 0%,#23aa5d 100%)" }}
      >
        <WavyTop color="#ffffff" />

        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-5 gap-8 items-center justify-items-center relative z-10">
          {[
            { label: "Organic", glyph: "🌿" },
            { label: "Natural", glyph: "🍃" },
            { label: "Organic Products", glyph: "🌱" },
            { label: "100% Pure", glyph: "✨" },
            { label: "No Preservatives", glyph: "🚫" },
          ].map((b, i) => (
            <div key={i} className="flex flex-col items-center gap-2 opacity-90 hover:opacity-100 transition-opacity">
              <div
                className="w-20 h-20 rounded-full border-2 border-white/60 flex items-center justify-center text-3xl"
                style={{ background: "rgba(255,255,255,0.15)" }}
              >
                {b.glyph}
              </div>
              <span className="text-xs uppercase tracking-widest font-semibold text-white/80">{b.label}</span>
            </div>
          ))}
        </div>
      ...
      {/* ══════════════════════════════════════
          7. FRANCHISE PRICING  (white bg)
      ══════════════════════════════════════ */}
      <section className="py-24 px-4 max-w-6xl mx-auto">
        <div className="text-center mb-16 relative">
          {/* Faded "WW" watermark like Frappé */}
          <span
            className="absolute inset-0 flex items-center justify-center text-[9rem] md:text-[14rem] font-black text-gray-100 select-none pointer-events-none z-0 leading-none"
            aria-hidden
          >
            WW
          </span>
          <div className="relative z-10">
            <span className="text-[#23aa5d] text-2xl">♥</span>
            <h2 className="text-3xl md:text-5xl font-light text-gray-800 mt-2 mb-2" style={{ fontFamily: "var(--font-quicksand), sans-serif" }}>Our Franchise Models</h2>
            <p className="text-[#23aa5d] text-sm italic" style={{ fontFamily: "cursive" }}>
          ...
          {[
            {
              ...
              price: "25",
              color: "#8ee3b0",
              features: ["Low initial investment", "Compact footprint", "High profit margins", "Full brand support", "Training provided"],
            },
            {
              title: "Express Outlet",
              price: "45",
              color: "#23aa5d",
              features: ["Expanded beverage menu", "FOCO operation setup", "Dedicated marketing team", "Centralised supply chain", "Priority support"],
              highlight: true,
            },
            {
              title: "Flagship Café",
              price: "85",
              color: "#8ee3b0",
              features: ["Full menu & food items", "Premium seating setup", "Priority distribution", "End-to-end management", "Brand ambassador events"],
            },
          ].map((plan, i) => (
            <div
              key={i}
              className={`rounded-2xl p-8 text-center text-white flex flex-col justify-between shadow-lg hover:shadow-2xl transition-all duration-300 relative overflow-hidden ${
                plan.highlight ? "md:-translate-y-4 scale-[1.02]" : ""
              }`}
              style={{ background: plan.color }}
            >
              {/* decorative fruit watermark */}
              <div className="absolute right-2 bottom-2 text-white/10 text-[7rem] leading-none select-none pointer-events-none">
                🍓
              </div>

              <div>
                <h3 className="text-base font-bold uppercase tracking-wider mb-5 text-white/90">{plan.title}</h3>
                <div className="flex items-start justify-center gap-1 mb-1">
                  <span className="text-xl font-bold mt-2">$</span>
                  <span className="text-6xl font-black">{plan.price}</span>
                </div>
                <div className="text-xs uppercase tracking-widest text-white/70 mb-8">monthly entry</div>
              ...
              <button className="bg-white text-gray-700 font-semibold py-2.5 px-8 rounded hover:bg-gray-50 transition-colors text-xs uppercase tracking-widest shadow">
                Buy Now
              </button>
            </div>
          ))}
